...
parse_qs([], Acc) ->
  lists:reverse(Acc);
parse_qs(String, Acc) ->
  case parse_qs_key(String) of
    {[], Key, Rest} ->
      {Value, Rest1} = parse_qs_value(Rest),
      parse_qs(Rest1, [{Key, Value} | Acc]);
    {Parents, Key, Rest} ->
      {Value, Rest1} = parse_qs_value(Rest),
      Acc1 = parse_qs_merge(Parents, Key, Value, Acc),
      parse_qs(Rest1, Acc1)
  end.
...
parse_qs_key([], Parents, Acc) ->
  {Parents, qs_revdecode(Acc), ""};
parse_qs_key([$= | Rest], Parents, Acc) ->
  {Parents, qs_revdecode(Acc), Rest};
parse_qs_key(Rest=[$; | _], Parents, Acc) ->
  {Parents, qs_revdecode(Acc), Rest};
parse_qs_key(Rest=[$& | _], Parents, Acc) ->
  {Parents, qs_revdecode(Acc), Rest};
parse_qs_key([?LBRACE | Rest], Parents, Acc) ->
  case parse_qs_parent(Rest) of
    {valid, Key, Rest1} ->
...
parse_qs_parent(String) ->
  parse_qs_parent(String, []).
parse_qs_parent([], Acc) ->
  {invalid, Acc, ""};
parse_qs_parent([$= | Rest], Acc) ->
  {invalid, Acc, Rest};
parse_qs_parent(Rest=[$; | _], Acc) ->
  {invalid, Acc, Rest};
parse_qs_parent(Rest=[$& | _], Acc) ->
  {invalid, Acc, Rest};
parse_qs_parent([?RBRACE | Rest], Acc) ->
  {valid, Acc, Rest};
parse_qs_parent([C | Rest], Acc) ->
  parse_qs_parent(Rest, [C | Acc]).
...
parse_qs_merge([], Key, Value, Acc) ->
  lists:ukeysort(1, [{Key, Value}] ++ Acc);
parse_qs_merge([H|T], Key, Value, Acc) ->
  case proplists:get_value(H, Acc) of
    undefined ->
      [{H, parse_qs_merge(T, Key, Value, []) ++ Acc}];
    PropList ->
      Merger = parse_qs_merge(T, Key, Value, PropList),
      lists:ukeysort(1, [{H, Merger}] ++ Acc)
  end.
